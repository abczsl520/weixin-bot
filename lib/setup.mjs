...
export async function interactiveSetup(config) {
  ...
  console.log('    7) Custom        (any OpenAI-compatible API)');
  console.log('    8) Echo mode     (no AI, just echo messages back)');
  console.log();

  const choice = await ask(rl, '  Choose provider [1-8]: ');

  const providers = {
    '1': 'openai', '2': 'claude', '3': 'gemini', '4': 'ollama',
    '5': 'codex', '6': 'claude-code', '7': 'custom', '8': 'echo',
  };
