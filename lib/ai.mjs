...
async function callCodex(config, userText) {
  ...
  try {
    const { stdout } = await execFileAsync(
      codexPath, ['-q', userText],
      { encoding: 'utf-8', timeout: 120000, env }
    );
    return stdout.trim() || '(Codex returned empty response)';
  } catch (err) {
...
async function callClaudeCode(config, userText) {
  ...
  try {
    const { stdout } = await execFileAsync(
      claudePath, ['-p', userText, '--model', model, '--dangerously-skip-permissions'],
      { encoding: 'utf-8', timeout: 120000, env }
    );
    return stdout.trim() || '(Claude Code returned empty response)';
  } catch (err) {
    const stderr = err.stderr?.toString() || '';
...
function findBinary(name) {
  // Only allow known binary names
  const allowed = ['codex', 'claude'];
  if (!allowed.includes(name)) return null;
  try {
    const cmd = process.platform === 'win32' ? 'where' : 'which';
    return execFileSync(cmd, [name], { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim().split('\n')[0];
  } catch {
    return null;
  }
}
...
function cleanupConversations() {
  if (conversations.size > 500) {
    ...
    for (const [key] of toRemove) conversations.delete(key);
  }
}
